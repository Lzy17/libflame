...
FLA_Error FLA_Copyt_external_hip( rocblas_handle handle, FLA_Obj A, void* A_hip, FLA_Obj B, void* B_hip )
{
	...
	err = hip_copyt( handle, A_mat, B_mat, width, height, dpitch, spitch, dtype);


	if ( err != hipSuccess )
	{
		fprintf( stderr, "Failure to perform D2D copy transpose in HIP: %d\n", err);
		return FLA_FAILURE;
	}
